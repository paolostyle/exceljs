...
class StopXform extends BaseXform {
  ...
  parseOpen(node) {
    ...
    switch (node.name) {
      case 'stop':
        this.model = {
          position: Number.parseFloat(node.attributes.position),
        };
        return true;
      case 'color':
    ...
    switch (node.name) {
      case 'gradientFill': {
        ...
        if (node.attributes.degree) {
          ...
          model.degree = Number.parseInt(node.attributes.degree, 10);
        } else if (node.attributes.type === 'path') {
          model.gradient = 'path';
          model.center = {
            left: node.attributes.left
              ? Number.parseFloat(node.attributes.left)
              : 0,
            top: node.attributes.top
              ? Number.parseFloat(node.attributes.top)
              : 0,
          };
          if (node.attributes.right !== node.attributes.left) {
            model.center.right = node.attributes.right
              ? Number.parseFloat(node.attributes.right)
              : 0;
          }
          if (node.attributes.bottom !== node.attributes.top) {
            model.center.bottom = node.attributes.bottom
              ? Number.parseFloat(node.attributes.bottom)
              : 0;
          }
        }
        return true;
