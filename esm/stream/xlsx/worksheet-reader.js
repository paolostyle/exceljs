          ...
          if (emitSheet) {
            switch (node.name) {
              ...
              case 'col':
                if (inCols) {
                  cols.push({
                    min: Number.parseInt(node.attributes.min, 10),
                    max: Number.parseInt(node.attributes.max, 10),
                    width: Number.parseFloat(node.attributes.width),
                    styleId: Number.parseInt(node.attributes.style || '0', 10),
                  });
                }
                break;
              ...
              case 'row':
                if (inRows) {
                  const r = Number.parseInt(node.attributes.r, 10);
                  row = new Row(this, r);
                  if (node.attributes.ht) {
                    row.height = Number.parseFloat(node.attributes.ht);
                  }
                  if (node.attributes.s) {
                    const styleId = Number.parseInt(node.attributes.s, 10);
              ...
              case 'c':
                if (row) {
                  c = {
                    ...
                    s: Number.parseInt(node.attributes.s, 10),
                    t: node.attributes.t,
                  };
                }
                break;
                  ...
                  if (c.f) {
                    ...
                    if (c.v) {
                      if (c.t === 'str') {
                        ...
                        cellValue.result = Number.parseFloat(c.v.text);
                      }
                    }
                    ...
                    switch (c.t) {
                      case 's': {
                        const index = Number.parseInt(c.v.text, 10);
                        if (sharedStrings) {
                      ...
                      case 'b':
                        cell.value = Number.parseInt(c.v.text, 10) !== 0;
                        break;

                      default:
                        if (utils.isDateFmt(cell.numFmt)) {
                          cell.value = utils.excelToDate(
                            Number.parseFloat(c.v.text),
                            properties.model?.date1904,
                          );
                        } else {
                          cell.value = Number.parseFloat(c.v.text);
                        }
                        break;
                    }
                  }
