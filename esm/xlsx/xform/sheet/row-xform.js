  ...
  parseOpen(node) {
    ...
    if (node.name === 'row') {
      ...
      const spans = node.attributes.spans
        ? node.attributes.spans
            .split(':')
            .map((span) => Number.parseInt(span, 10))
        : [undefined, undefined];
      const model = (this.model = {
        number: Number.parseInt(node.attributes.r, 10),
        min: spans[0],
        max: spans[1],
        cells: [],
      });
      if (node.attributes.s) {
        model.styleId = Number.parseInt(node.attributes.s, 10);
      }
      ...
      if (node.attributes.ht) {
        model.height = Number.parseFloat(node.attributes.ht);
      }
      if (node.attributes.outlineLevel) {
        model.outlineLevel = Number.parseInt(node.attributes.outlineLevel, 10);
      }
