...
class VmlShapeXform extends BaseXform {
  ...
  parseClose(name) {
    ...
    switch (name) {
      case this.tag:
        this.model.margins.inset = this.map['v:textbox'].model?.inset;
        this.model.protection = this.map['x:ClientData'].model?.protection;
        this.model.anchor = this.map['x:ClientData'].model?.anchor;
        this.model.editAs = this.map['x:ClientData'].model?.editAs;
        return false;
      default:
        return true;
    }
  }
  static V_SHAPE_ATTRIBUTES = (model, index) => ({
