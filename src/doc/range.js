...
class Range {
  ...
  setTLBR(...args) {
    if (args.length < 4) {
      const [_tl, _br, s] = args;
      const tl = colCache.decodeAddress(_tl);
      const br = colCache.decodeAddress(_br);
      this.model = {
  ...
  decode(argv) {
    switch (argv.length) {
      ...
      default:
        throw new Error(
          `Invalid number of arguments to _getDimensions() - ${argv.length}`,
        );
    }
  }
