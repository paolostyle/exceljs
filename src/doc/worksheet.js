...
class Worksheet {
  ...
  set columns(value) {
    ...
    this._headerRowCount = value.reduce((pv, cv) => {
      const headerCount = (cv.header && 1) || cv.headers?.length || 0;
...
Please leave feedback at https://github.com/exceljs/exceljs/discussions/2575`,
  ...
  get model() {
    ...
    model.mergeCells = [];
    _.each(this._merges, (merge) => {
      model.mergeCells.push(merge.range);
    });

    // TODO: remove in next major version - this property doesn't even work
    model.merges = model.mergeCells;
