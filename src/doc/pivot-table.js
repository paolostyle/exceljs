...
export function makePivotTable(worksheet, model) {
  // Example `model`:
  // {
...
function validate(worksheet, model) {
  ...
  for (const name of [...model.rows, ...model.columns, ...model.values]) {
    if (!isInHeaderNames[name]) {
      throw new Error(
        `The header name "${name}" was not found in ${model.sourceSheet.name}.`,
      );
    }
  }
...
function makeCacheFields(worksheet, fieldNamesWithSharedItems) {
  ...
  for (const columnIndex of range(1, names.length)) {
    ...
    const sharedItems = nameToHasSharedItems[name]
      ? aggregate(columnIndex)
      : null;
